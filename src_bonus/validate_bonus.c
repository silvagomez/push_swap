...
/*   Updated: 2023/09/28 03:24:53 by dsilva-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_str_av(char *s)
{
	...
	if (!s || !*s)
		error_terminate("ERROR: null is no acceptable.");
	idx = 0;
	if (s[idx] == ' ')
		error_terminate("ERROR: can not start with space.");
	while (s[idx])
	{
		if (!ft_isdigit(s[idx]) && s[idx] != '+' && s[idx] != '-' && \
				s[idx] != ' ')
			error_terminate("ERROR: unacceptable character.");
		idx++;
	}
}
...
void	validate_is_duplicate(char **str_num, size_t size, int event)
{
	...
	if (!nums)
		error_validate(str_num, event, "ERROR: malloc long array nums.");
	idx = 0;
	while (str_num[idx])
	{
	...
	{
		free(nums);
		error_validate(str_num, event, "ERROR: duplicate number found.");
	}
	free(nums);
}
...
void	validate_av(char **str_num, int event)
{
	...
	while (str_num[idx])
	{
		...
		{
			ft_putstr_fd("ERROR: unvalid format, only can be digits, ", 2);
			error_validate(str_num, event, "+digit or -digit.");
		}
		num = ft_atol(str_num[idx]);
		if (num < -2147483648 || num > 2147483647)
			error_validate(str_num, event, "ERROR: out of range max|min int");
		idx++;
	}
